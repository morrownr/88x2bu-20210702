...
static unsigned char *__nat25_find_pppoe_tag(struct pppoe_hdr *ph, unsigned short type)
{
	...
	start_ptr = cur_ptr = (unsigned char *)((unsigned char *)(ph + 1));
...
static int __nat25_add_pppoe_tag(struct sk_buff *skb, struct pppoe_tag *tag)
{
	...
	memmove(((unsigned char *)((unsigned char *)(ph + 1)) + data_len), (unsigned char *)((unsigned char *)(ph + 1)), ntohs(ph->length));
	ph->length = htons(ntohs(ph->length) + data_len);
	memcpy((unsigned char *)((unsigned char *)(ph + 1)), tag, data_len);
	return data_len;
}
			...
			if (ph->sid == 0) {	/* Discovery phase according to tag */
				if (ph->code == PADI_CODE || ph->code == PADR_CODE) {
					if (priv->ethBrExtInfo.addPPPoETag) {
						...
						pMagic = (unsigned short *)((unsigned char *)(tag + 1));
